...
"""
    ...
    def _on_update_kernel(self):
        ...
        from PyQt6.QtWidgets import QMessageBox
        import subprocess
        from core.paths import WEBUI_DIR, BASE_DIR
        ...
        if msg.exec() == QMessageBox.StandardButton.Ok:
            ...
            try:
                # 使用统一的路径管理
                webui_dir = WEBUI_DIR
                # 如果 webui 目录不存在或没有 .git，回退到项目根目录
                if not os.path.exists(webui_dir) or not os.path.exists(os.path.join(webui_dir, ".git")):
                    webui_dir = BASE_DIR
                
                if os.path.exists(webui_dir):
                    # 设置 git 环境变量，禁用代理和 SSL 验证（解决网络连接问题）
                    env = os.environ.copy()
                    ...
                    # 取消 git 配置中的代理设置（解决 SOCKS5 代理问题）
                    self.append_line("[更新内核] 取消 git 代理配置...")
                    subprocess.run(
                        ["git", "-C", webui_dir, "config", "--unset", "http.proxy"],
                    ...
                    subprocess.run(
                        ["git", "-C", webui_dir, "config", "--unset", "https.proxy"],
                        stdout=subprocess.PIPE,
                        stderr=subprocess.STDOUT,
                        text=True,
                        env=env
                    ...
                    subprocess.run(
                        ["git", "-C", webui_dir, "config", "--global", "--unset", "http.proxy"],
                        stdout=subprocess.PIPE,
                        stderr=subprocess.STDOUT,
                        text=True,
                        env=env
                    ...
                    subprocess.run(
                        ["git", "-C", webui_dir, "config", "--global", "--unset", "https.proxy"],
                        stdout=subprocess.PIPE,
                        stderr=subprocess.STDOUT,
                        text=True,
                        env=env
