...
class SettingsTab(QWidget):
    ...
    def _group_attention(self) -> QGroupBox:
        ...
        # 说明文字
        hint = QLabel("💡 优先级：SageAttention > FlashAttention > xFormers > PyTorch原生")
        ...
        for key, text in [
            ("enable_sage",     "启用 SageAttention (推荐)"),
    ...
    def _load_config(self):
        ...
        for key in ["listen", "autolaunch", "api", "share", "show_console",
                    ...
                    "lowvram", "no_half", "no_half_vae", "precision_full",
        ...
        for key, default in [
            ("enable_sage", True),     # 默认启用 SageAttention
            ...
            if key in c:
                ...
                if old_key in c:
                    # 旧配置：disable_sage=True 表示禁用，所以 enable_sage=False
    ...
    def apply_to_config(self, config: dict):
        ...
        # 注意力机制：保存启用状态
        for key in ["enable_sage", "enable_flash", "enable_xformers"]:
            if key in self._widgets:
                config[key] = self._widgets[key].isChecked()
