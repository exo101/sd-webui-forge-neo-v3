...
def build_args(config: dict) -> list[str]:
    ...
    - cuda_stream: CUDA流优化
    - enable_sage/enable_flash/enable_xformers: 注意力机制开关（默认全部启用）
    - lowvram: 显存优化模式
    - gpu_device: GPU设备选择 (通过CUDA_VISIBLE_DEVICES环境变量设置)
    
    注意力机制说明:
    - 默认全部启用，按优先级自动选择：SageAttention > FlashAttention > xFormers > PyTorch原生
    - 不启用时会添加 --disable-* 参数
    - 遇到NaN错误时，可以禁用SageAttention
    ...
    if not config.get("enable_sage", True):     args.append("--disable-sage")
