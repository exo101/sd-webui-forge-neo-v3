...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        def check_keyword():
            ...
            if entry:
                kw = entry[0]
                src = 'custom-mappings.txt' if entry[2]==1 else 'model-keyword.txt (默认数据库)'
                return f"文件名={model_ckpt}\n哈希值={model_hash}\n关键词={kw}\n匹配来源: {src}"
            else:
                return f"文件名={model_ckpt}\n哈希值={model_hash}\n未找到匹配"
        ...
        def add_lora_keyword(txt, lora_model):
            ...
            lora_keywords = get_lora_keywords(lora_model)
            return ['已添加: ' + insert_line, gr.Dropdown.update(choices=lora_keywords)]
        ...
        with gr.Group():
            with gr.Accordion('模型关键词', open=False):

                is_enabled = gr.Checkbox(label='启用模型关键词', value=cb_enabled)
                ...
                with gr.Accordion('LORA', open=True):
                    ...
                    with gr.Row(equal_height=True):
                        ...
                        lora_multiplier = gr.Slider(minimum=0,maximum=2, step=0.01, value=cb_lora_multiplier, label="权重")
                    with gr.Row(equal_height=True):
                    ...
                    info = gr.HTML("<p style=\"margin-bottom:0.75em\">为选中的LORA模型添加自定义关键词(触发词)映射。</p>")
                    lora_text_input = gr.Textbox(placeholder="关键词或用 | 分隔的多个关键词", label="关键词(触发词)")
                    with gr.Row():
                        add_mappings = gr.Button(value='保存')
                        delete_mappings = gr.Button(value='删除')
                    lora_text_output = gr.Textbox(interactive=False, label='结果')
                ...
                with gr.Accordion('添加自定义映射', open=False):
                    info = gr.HTML("<p style=\"margin-bottom:0.75em\">为当前模型添加自定义关键词(触发词)映射。自定义映射保存在 extensions/model-keyword/custom-mappings.txt</p>")
                    text_input = gr.Textbox(placeholder="关键词或用 | 分隔的多个关键词", label="关键词(触发词)")
                    with gr.Row():
                        check_mappings = gr.Button(value='检查')
                        add_mappings = gr.Button(value='保存')
                        delete_mappings = gr.Button(value='删除')

                    text_output = gr.Textbox(interactive=False, label='结果')
