...
export default defineComponent({
  ...
  methods: {
    ...
    async loadCanvasFromFrameMessage(message: IncomingFrameMessage) {
      ...
      const hasPose =
        !!message.poseURL ||
        (Array.isArray(message.poses) ? message.poses.length > 0 : !!message.poses);
      ...
      if (hasPose) {
        ...
        // No pose provided (e.g. the user opened the editor without first
        // running the preprocessor). Keep the default person so the user can
        // create a pose from scratch.
        this.addDefaultPerson();
      }
