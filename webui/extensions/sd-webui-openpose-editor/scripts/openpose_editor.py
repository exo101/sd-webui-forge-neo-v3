...
def mount_openpose_api(_: gr.Blocks, app: FastAPI):
    if not os.path.isdir(DIST_DIR):
        print(
            "[openpose-editor] dist directory not found at "
            f"'{DIST_DIR}'. The OpenPose Editor will be unavailable. "
            "Build it with `npm install && npm run build` (or let the "
            "auto-update download it on next launch)."
        )
        return
    ...
    if not getattr(shared.cmd_opts, "disable_openpose_editor_auto_update", False):
        try:
            update_app()
        except Exception as e:  # Never let auto-update break WebUI startup.
            print(f"[openpose-editor] auto-update skipped: {e}")
