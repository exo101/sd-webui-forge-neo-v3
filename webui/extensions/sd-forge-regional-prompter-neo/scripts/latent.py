        ...
        if not "Pro" in self.mode:
            ...
            if indrebuild:
                if "Ran" in self.mode:
                    ...
                    if "Mask" in self.mode or "Visual" in self.mode:
                        masks = (self.regmasks,self.regbase)
                    else:
                        masks = self.aratios  #makefilters(c,h,w,masks,mode,usebase,bratios,indmask = None)
                    self.filters = makefilters(x.shape[1], x.shape[2], x.shape[3],masks, self.mode, self.usebase, self.bratios, "Mas" in self.mode or "Visual" in self.mode)
