...
class LipsyncPipeline(DiffusionPipeline):
    ...
    def affine_transform_video(self, video_frames):
        ...
        pbar = tqdm.tqdm(total=len(video_frames), desc="Affine transforming faces")
        for idx, frame in enumerate(video_frames):
            try:
                face, box, affine_matrix = self.image_processor.affine_transform(frame)
                loop_video_frames.append(frame)
                loop_faces.append(face)
                loop_boxes.append(box)
                loop_affine_matrices.append(affine_matrix)
                pbar.update(1)
            except RuntimeError as e:
                if "Face not detected" in str(e):
                    # 跳过没有人脸的帧，记录警告但不中断处理
                    print(f"Warning: Face not detected in frame {idx}, skipping this frame.")
                    continue
                else:
                    # 重新抛出其他运行时错误
                    raise e
        ...
        # 检查是否所有帧都没有检测到人脸
        if len(loop_faces) == 0:
            raise RuntimeError("Face not detected in any frame. Please ensure your video contains clear frontal faces.")

        faces = torch.stack(loop_faces)
        return faces, loop_boxes, loop_affine_matrices
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 如果没有提供mask_image_path或者提供的路径不存在，则使用默认路径
        if mask_image_path is None or not os.path.exists(mask_image_path):
            mask_image_path = os.path.join(os.path.dirname(__file__), "..", "utils", "mask.png")
        ...
        for i in tqdm.tqdm(range(num_inferences), desc="Doing inference..."):
            ...
            # 9. Denoising loop
            num_warmup_steps = len(timesteps) - num_frames * self.scheduler.order
            with self.progress_bar(range(num_frames)) as progress:
