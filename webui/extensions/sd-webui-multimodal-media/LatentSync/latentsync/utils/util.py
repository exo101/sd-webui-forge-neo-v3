...
def write_video(video_frames: np.ndarray, audio_samples: np.ndarray, video_output_path: str, fps: int, audio_sample_rate: int = 16000):
    try:
        import soundfile as sf
        import tempfile
        import os
        ...
        # 保存音频到临时文件
        sf.write(tmp_audio_path, audio_samples, audio_sample_rate)
        ...
        try:
            ...
            print(f"警告：无法直接写入视频: {video_write_error}")
            # 尝试不带额外参数的写入方式
            try:
                ...
                print(f"警告：备用写入方法也失败了: {fallback_error}")
                # 使用OpenCV作为最终回退方案
                write_video_cv2(video_output_path, video_frames, fps)
                # 手动合并音频
                merge_audio_video_ffmpeg(video_output_path, tmp_audio_path)
                return  # 成功使用回退方法
        ...
        try:
            from subprocess import run
            import os
            temp_video_path = video_output_path + ".temp.mp4"
            os.rename(video_output_path, temp_video_path)
            ...
            print(f"警告：无法合并音频和视频: {ffmpeg_error}")
            # 如果ffmpeg失败，保留无音频的视频
            pass
        
    except Exception as e:
        # 如果上述方法失败，至少保存没有音频的视频
        print(f"警告：无法添加音频到视频: {e}")
        try:
            with imageio.get_writer(
                video_output_path,
                fps=fps,
            ...
            print(f"警告：无法写入视频: {video_write_error}")
            # 尝试最简单的写入方式
            try:
                with imageio.get_writer(video_output_path, fps=fps) as writer:
                    for video_frame in video_frames:
                ...
                print(f"警告：所有视频写入方法都失败了: {final_error}")
                # 使用OpenCV作为最终回退方案
                write_video_cv2(video_output_path, video_frames, fps)
...
def merge_audio_video_ffmpeg(video_path: str, audio_path: str):
    """使用ffmpeg手动合并音频和视频"""
    try:
        from subprocess import run
        import os
        ...
        print(f"警告：无法使用ffmpeg合并音频和视频: {e}")
        # 恢复原始视频文件
        try:
