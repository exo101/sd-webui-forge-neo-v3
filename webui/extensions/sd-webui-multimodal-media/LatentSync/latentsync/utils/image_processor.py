...
def load_fixed_mask(resolution: int, mask_image_path=None) -> torch.Tensor:
    # 如果 mask_image_path 是一个张量，则直接返回它（适配到指定分辨率）
    if isinstance(mask_image_path, torch.Tensor):
        # 如果张量的形状不是 [C, H, W]，我们需要调整它
        if mask_image_path.dim() == 3 and mask_image_path.shape[-1] == resolution and mask_image_path.shape[-2] == resolution:
            return mask_image_path
        # 如果需要调整大小，我们可以添加适当的处理
        # 但现在我们简单地假设它已经是正确的格式
        return mask_image_path
    
    # 如果没有提供mask_image_path，则使用默认路径
    ...
    if mask_image is None:
        raise RuntimeError(f"Failed to load mask image: {mask_image_path}")
...
class ImageProcessor:
    def __init__(self, resolution: int = 512, device: str = "cpu", mask_image=None):
        ...
        if mask_image is None:
            ...
            # 如果 mask_image 不是 None，则直接使用它作为掩码图像
            # 在这种情况下，mask_image 应该已经是一个张量了
