...
def create_layer_separation_ui():
    ...
    with gr.Group(), gr.Accordion("See-Through 图层分离为PSD文件", open=True, elem_id="see-through-accordion"):

        with gr.Row():
            see_through_enabled = gr.Checkbox(label="启用See-Through", value=True)

        with gr.Row(visible=True) as see_through_options:
            with gr.Column():
                gr.Markdown("### 图像输入")
            ...
            try:
                ...
                try:
                    if segmentation_mode == "场景分割 (SAM)":
                        ...
                        logger.info(f"[Scene Segmentation] 分割所有目标")
                        ...
                        if process.returncode == 0:
                            ...
                            # 收集所有图层文件作为预览
                            import glob
                            saved_dir = os.path.join(output_dir, f"uploaded_image_{timestamp}")
                            preview_images = None
            ...
            for i, file_info in enumerate(batch_files):
                ...
                try:
                    ...
                    if timed_out:
                        ...
                        # 收集所有图层文件作为预览
                        saved_dir = os.path.join(output_dir, f"batch_{basename}_{timestamp}")
                        if os.path.exists(saved_dir):
                            png_files = sorted(
                                glob.glob(os.path.join(saved_dir, "*.png")),
                                key=os.path.getmtime, reverse=True
