...
def _fix_init_images_for_flux(p) -> None:
    # No longer used - fix is applied in process_batch instead.
    pass
...
class Script(scripts.Script):
    ...
    def process_batch(self, p, *args, **kwargs):
        """Fix Flux-family img2img for both batch_size > 1 and n_iter > 1.

        Two separate problems, same root cause (ini_latent consumed on first use):

        Problem 1 - n_iter > 1:
            init() runs once before the n_iter loop. encode_first_stage() sets
            sd_model.ini_latent. get_learned_conditioning() consumes it (sets
            to None) on iteration 0. Iterations 1+ find ini_latent=None.
            Fix: restore ini_latent at the start of each iteration > 0.

        Problem 2 - batch_size > 1 with unique prompts:
            prompt_parser.get_learned_conditioning() loops over each unique
            prompt and calls model.get_learned_conditioning() once per unique
            prompt. The first call consumes ini_latent and sets
            dynamic_args["ref_latents"] = [ini_latent]. The second call finds
            ini_latent=None and overwrites dynamic_args["ref_latents"] = [].
            The final ref_latents after setup_conds() is therefore empty.
            Fix: wrap model.get_learned_conditioning so ini_latent is restored
            before every positive-prompt call. Cleaned up in postprocess_batch.
        """
        sd_model = getattr(p, "sd_model", None)
        if sd_model is None or not hasattr(sd_model, "ini_latent"):
            return  # Not a Flux-family model.

        init_latent = getattr(p, "init_latent", None)
        if init_latent is None:
            return  # Not an img2img run.

        cached_latent = init_latent.cpu()

        # Always restore ini_latent at the start of each iteration.
        # For n_iter > 1: iteration 0 already has it from init(), but restoring
        # it here is harmless and keeps the logic uniform.
        sd_model.ini_latent = cached_latent

        # For batch_size > 1 with unique prompts, we also need ini_latent
        # restored before the 2nd, 3rd, ... unique-prompt conditioning calls
        # inside setup_conds(). Install an instance-level wrapper that does
        # this, then let postprocess_batch clean it up.
        if getattr(sd_model, "_dp_glc_patched", False):
            return  # Already installed (safety guard).

        original_glc = sd_model.__class__.get_learned_conditioning

        def _wrapped_glc(prompt):
            if not getattr(prompt, "is_negative_prompt", False):
                sd_model.ini_latent = cached_latent
            return original_glc(sd_model, prompt)

        sd_model.get_learned_conditioning = _wrapped_glc
        sd_model._dp_glc_patched = True
        logger.debug(
            f"[Dynamic Prompts] Installed ini_latent restore wrapper "
            f"(batch_number={kwargs.get('batch_number', 0)}, batch_size={p.batch_size})."
        )

    def postprocess_batch(self, p, *args, **kwargs):
        """Remove the ini_latent wrapper installed by process_batch."""
        sd_model = getattr(p, "sd_model", None)
        if sd_model is None:
            return
        if getattr(sd_model, "_dp_glc_patched", False):
            try:
                del sd_model.get_learned_conditioning
            except AttributeError:
                pass
            try:
                del sd_model._dp_glc_patched
            except AttributeError:
                pass
            logger.debug("[Dynamic Prompts] Removed ini_latent restore wrapper.")
