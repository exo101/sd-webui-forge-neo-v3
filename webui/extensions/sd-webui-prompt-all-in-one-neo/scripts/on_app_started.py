...
from scripts.physton_prompt.hunyuan import initialize as hunyuan_initialize
...
def on_app_started(_: gr.Blocks, app: FastAPI):
    ...
    @app.post("/physton_prompt/hunyuan_initialize")
    async def _hunyuan_initialize(request: Request):
        try:
            try:
                data = await request.json()
            except Exception:
                data = {}
            model_key = data.get('model') if isinstance(data, dict) else None
            hunyuan_initialize(model_key=model_key, reload=True)
            return {"success": True}
        except Exception as e:
            return {"success": False, 'message': str(e)}
