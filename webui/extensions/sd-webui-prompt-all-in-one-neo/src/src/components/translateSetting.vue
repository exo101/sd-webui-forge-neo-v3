        ...
        <div class="physton-prompt-translate-setting" v-if="isOpen" @click="onCloseClick">
            <div class="translate-setting-main" @click.stop>
                ...
                <div class="translate-setting-content" @click.stop>
                    ...
                    <div class="setting-line" v-if="apiItem.key === 'hunyuan'">
                        <div class="line-title">{{ getLang('initialize') }}</div>
                        <div class="line-content">
                            <div class="hover-scale-120 test-btn" @click="onHunyuanInitialize">
                                <icon-svg v-if="hunyuanLoading" name="loading"/>
                                <template v-else>{{ getLang('initialize') }}</template>
                            </div>
                            <p class="common-red" v-html="getLang('download_model_desc')"></p>
                            <p class="common-red" v-html="getLang('download_model_desc2')"></p>
                    ...
                    <div class="setting-line" v-if="apiItem.key === 'hunyuan' && hunyuanMessage">
                        <div class="line-title"></div>
                        <div class="line-content">
                            <div :class="[hunyuanSuccess ? '' : 'common-red']">{{ hunyuanMessage }}</div>
                        </div>
                    </div>
                    <div class="setting-line">
...
export default {
    ...
    data() {
        return {
            ...
            mbart50Message: '',
            hunyuanLoading: false,
            hunyuanSuccess: false,
            hunyuanMessage: '',
        }
    },
    computed: {
...
Github: {{name}}`
        ...
        onHunyuanInitialize() {
            this.hunyuanLoading = true
            this.hunyuanSuccess = false
            this.hunyuanMessage = ''
            const modelConfig = this.configs.find(item => item.key === 'model')
            const model = modelConfig ? modelConfig.value : null
            this.gradioAPI.hunyuanInitialize(model).then(res => {
                this.hunyuanLoading = false
                if (res.success) {
                    this.hunyuanSuccess = true
                    this.hunyuanMessage = this.getLang('initialize_finished')
                } else {
                    this.hunyuanMessage = this.getLang('initialize_failed') + ': ' + res.message
                }
            }).catch(err => {
                this.hunyuanLoading = false
                this.hunyuanMessage = err.message
            })
        },
    },
}
