...
export default {
    ...
    mounted() {
        ...
        this.gradioAPI.getConfigWithRetry({attempts: 30, initialDelay: 250, maxDelay: 2000}).then(res => {
            ...
            this.$toastr.error("Connection to backend API service failed after automatic retries. Please check the WebUI console/log and restart the WebUI if necessary.<br/>自动重试后仍无法连接到后台接口服务，请检查 WebUI 控制台/日志，必要时重启 WebUI。<br/><br/>" + err.message, globals.shortName, {timeOut: 20000})
            console.log(err)
        })
    },
    methods: {
