...
def create_override_settings_dropdown(tabname, row):
    # allow_custom_value=True 防止 Gradio 5 在 choices 为空时前端发送空字符串
    # 导致 preprocess 报错 "Value: '' is not in the list of choices: []"
    dropdown = gr.Dropdown([], label="Override settings", visible=False, elem_id=f"{tabname}_override_settings", multiselect=True, allow_custom_value=True, value=[])
...
def create_ui():
    ...
    # 注册纵横比设置（必须在 UI 创建前，因为 UI 中需要读取这些选项）
    aspect_ratio.register_settings()
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        with gr.Tab("Generation", id="txt2img_generation") as txt2img_generation_tab, ResizeHandleRow(equal_height=False):
            with ExitStack() as stack:
                ...
                # Parameters to collapse into the "参数设置" accordion
                _params_categories = {"sampler", "seed", "cfg", "batch"}

                for category in ordered_ui_categories():
                    if category in _params_categories:
                        continue

                    if category == "prompt":
                        # Styles are deferred into the params accordion
                        toprow.create_inline_toprow_prompts(include_styles=False)
                        ...
                        with gr.Row(elem_id="txt2img_accordions", elem_classes="accordions"):
                            with InputAccordion(False, label="Hires. fix", elem_id="txt2img_hr") as enable_hr:
                                ...
                                with FormRow(elem_id="txt2img_hires_fix_row1", variant="compact"):
                                    ...
                                    denoising_strength = gr.Slider(minimum=0.0, maximum=1.0, step=0.05, label="重绘强度", value=0.6, elem_id="txt2img_denoising_strength")
                    ...
                    if category not in {"accordions", "cfg"} and category not in _params_categories:
                        scripts.scripts_txt2img.setup_ui_for_section(category)

                # ========== 参数设置折叠模块 ==========
                # 注意：open=True 确保 Gradio 5 前端正确初始化组件值，
                # 否则关闭 accordion 内的 slider/dropdown 会发送 -1/空字符串
                with gr.Accordion("参数设置", open=True, elem_id="txt2img_params_accordion"):
                    # 提示词工具（粘贴/清除/应用样式）+ 提示词预设
                    with gr.Row(elem_classes=["toprow-compact-stylerow"]):
                        with gr.Column(elem_classes=["toprow-compact-tools"]):
                            toprow.create_tools_row()
                        with gr.Column():
                            toprow.create_styles_ui()

                    for category in ordered_ui_categories():
                        if category not in _params_categories:
                            continue
                        ...
                        # sampler, seed and any other params-section scripts are
                        # rendered here once (cfg already rendered above)
                        if category != "cfg":
                            scripts.scripts_txt2img.setup_ui_for_section(category)
            ...
            with ExitStack() as stack:
                ...
                def add_copy_image_controls(tab_name, elem):
                    with gr.Row(variant="compact", elem_id=f"img2img_copy_to_{tab_name}"):
                        for title, name in zip(["发送到图生图", "发送到素描", "发送到局部重绘", "发送到局部重绘素描"], ["img2img", "sketch", "inpaint", "inpaint_sketch"]):
                            if name == tab_name:
                ...
                for category in ordered_ui_categories():
                    ...
                    if category == "image":
                        ...
                        denoising_strength = gr.Slider(minimum=0.0, maximum=1.0, step=0.01, label="重绘强度", value=0.6, elem_id="img2img_denoising_strength")
                ...
                # 注意：open=True 确保 Gradio 5 前端正确初始化组件值，
                # 否则关闭 accordion 内的 slider/dropdown 会发送 -1/空字符串
                with gr.Accordion("参数设置", open=True, elem_id="img2img_params_accordion"):
                    # 提示词工具（粘贴/清除/应用样式）+ 提示词预设
                    with gr.Row(elem_classes=["toprow-compact-stylerow"]):
                        with gr.Column(elem_classes=["toprow-compact-tools"]):
                            toprow.create_tools_row()
    ...
    with gr.Blocks(analytics_enabled=False) as pnginfo_interface:
        with ResizeHandleRow(equal_height=False):
            # 左侧：上传区 + 图像对比区（紧凑布局，最大化利用空间）
            with gr.Column(scale=3):
                # 双图并排上传，紧凑高度
                with gr.Row(elem_id="pnginfo_upload_row"):
                    image_a = gr.Image(
                ...
                # 图像对比面板：响应式宽度，居中，无多余留白
                with gr.Column(variant="panel", elem_id="img_comp_panel"):
                    ...
                    # 紧凑控制行：对比 + 交换 + 透明度 + 方向 + 缩放
                    with gr.Row(elem_id="img_comp_utils"):
                        comp_btn = gr.Button("对比上传", elem_id="img_comp_btn", scale=2, variant="primary")
                        swap_btn = gr.Button("交换", elem_id="img_comp_swap", scale=1)
                        gr.Slider(
                            label="透明度",
                        ...
                        ToolButton(value="\U00002795", elem_id="icomp_in", tooltip="放大")
                        ToolButton(value="\U0001f504", elem_id="icomp_reset", tooltip="重置缩放")
                        ToolButton(value="\U00002796", elem_id="icomp_out", tooltip="缩小")

            # 右侧：PNG 信息 + 参数按钮 + 差异对比
            with gr.Column(scale=2, variant="panel", elem_id="pnginfo_info_panel"):
                html = gr.HTML()
                ...
                for tabname, button in buttons.items():
                    parameters_copypaste.register_paste_params_button(
                        parameters_copypaste.ParamBinding(
                            ...
                            source_image_component=image_a,
                        )
                    )
        ...
        image_a.change(image_comparison.img2info, [image_a, image_b], [infotextA, infotextB])
        image_b.change(image_comparison.img2info, [image_a, image_b], [infotextA, infotextB])

        def _js_compare(mode: str) -> str:
            return f'() => {{ ImgCompLoader.loadImage("{mode}"); }}'

        comp_btn.click(fn=None, **image_comparison._gjs(_js_compare("upload")))

        swap_btn.click(fn=None, **image_comparison._gjs("() => { ImgCompLoader.swapImage(); }"))
        dir_cb.change(fn=None, **image_comparison._gjs("() => { ImageComparator.reset(); }"))
    ...
    image_comparison.register_settings()
    ...
    shared.gradio_head = canvas_head + """
<script>
if (!sessionStorage.getItem('physton_icon_refreshed')) {
    sessionStorage.setItem('physton_icon_refreshed', '1');
    setTimeout(function() { location.reload(); }, 2000);
}
// Apply saved theme ASAP (before Gradio's Svelte mounts) to prevent
// the default theme from flashing and Svelte from overwriting the class.
(function() {
    try {
        var saved = localStorage.getItem('sd-webui-theme');
        if (saved) {
            document.documentElement.classList.add(saved);
            var apply = function() { document.body && document.body.classList.add(saved); };
...
// First: fetch and inject latest theme.css directly (bypasses all caching)
// CRITICAL: path must NOT have "webui/" prefix — Gradio's file whitelist
...
(function() {
    fetch('/gradio_api/file=theme.css?v=82&t=' + Date.now(), { cache: 'no-store' })
        ...
        .then(function(css) {
            ...
            console.log('[SD] theme.css injected directly, length:', css.length);
        })
        .catch(function(e) { console.error('[SD] Failed to fetch theme.css:', e); });
})();
// Then: load sidebar.js via fetch+eval to bypass browser script caching.
// Same path rule: NO "webui/" prefix.
(function() {
    fetch('/gradio_api/file=javascript/sidebar.js?v=85&t=' + Date.now(), { cache: 'no-store' })
        .then(function(r) { return r.text(); })
        .then(function(code) {
            try {
                // Execute in global scope
                (0, eval)(code);
                console.log('[SD] sidebar.js loaded and executed via fetch+eval');
            } catch(e) {
                console.error('[SD] sidebar.js eval error:', e);
            }
        })
        .catch(function(e) { console.error('[SD] Failed to fetch sidebar.js:', e); });
})();
</script>
"""

    with gr.Blocks(analytics_enabled=False, title="Stable Diffusion", theme=shared.gradio_theme, head=shared.gradio_head) as demo:
